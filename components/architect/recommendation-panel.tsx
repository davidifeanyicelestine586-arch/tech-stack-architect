...
import { Check, Eye, Plus, SearchCheck, X, ChevronDown } from "lucide-react";
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 px-1">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="gap-1 text-[10px]"><SearchCheck className="size-3" aria-hidden="true" /> Matched to your project</Badge>
          </div>
        ...
        <Button variant="outline" className="h-11 gap-1.5 text-xs" onClick={addAllCompatibleRecommendations} disabled={compatibleCount === 0}><Plus className="size-3.5" aria-hidden="true" /> Add All Compatible ({compatibleCount})</Button>
      </div>
              ...
              <Card key={component.id} className="overflow-hidden transition-colors hover:border-primary/40">
                <CardHeader className="gap-2 pb-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 text-xs font-semibold text-primary">{recommendation.score}% fit</div>
                  </div>
                ...
                <CardContent className="pt-0">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border/50 pt-3">
                    <Button variant={isSelected ? "secondary" : "default"} size="sm" className="min-h-11 gap-1.5 text-[10px]" onClick={() => addRecommendation(component.id)} disabled={isSelected}>
                      {isSelected ? <Check className="size-3.5" aria-hidden="true" /> : <Plus className="size-3.5" />}{isSelected ? "Added to Stack" : "Add to Stack"}
                    </Button>
                    <Button variant="outline" size="sm" className="min-h-11 gap-1.5 text-[10px]" onClick={() => setDetailComponent(component)}><Eye className="size-3.5" aria-hidden="true" /> Details</Button>
                    {!isSelected && <Button variant="ghost" size="sm" className="min-h-11 gap-1.5 text-[10px] text-muted-foreground" onClick={() => ignoreRecommendation(component.id)}><X className="size-3.5" aria-hidden="true" /> Not for me</Button>}
                  </div>
                </CardContent>
              </Card>
            );
