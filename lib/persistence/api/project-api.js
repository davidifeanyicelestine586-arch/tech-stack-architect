...
const errorResponse = (error) => {
  const code = error instanceof PersistenceError || error instanceof RequestGuardError
    ? error.code
    : "DATABASE_FAILURE";
  return {
    ok: false,
...
const response = (payload, status, sessionId, isProduction, retryAfterSeconds = null) => {
...
export const createFailureResponse = (error, isProduction = false) => {
  ...
  return response(errorResponse(error), statusForCode(code), null, isProduction, retryAfterSeconds);
};
...
const run = async (
  ...
  try {
    ...
    if (logger && (error instanceof RequestGuardError || !(error instanceof PersistenceError))) logger(error);
