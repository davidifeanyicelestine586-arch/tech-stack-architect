...
const request = async (fetcher, path, init, getAccessToken) => {
  let response;
  try {
    const accessToken =
...
const assertRecord = (record) => {
  if (!record || typeof record !== "object" || Array.isArray(record)) {
    throw new ProjectPersistenceApiError("MALFORMED_RECORD", safeMessages.MALFORMED_RECORD);
  }
  if (!isUuid(record.id)) {
    throw new ProjectPersistenceApiError("MALFORMED_RECORD", safeMessages.MALFORMED_RECORD);
  }
...
export const createProjectPersistenceClient = (
  ...
  getAccessToken = typeof window === "undefined" ? null : getDefaultAccessToken
) => {
  ...
  const call = (path, init) => request(fetcher, path, init, getAccessToken);

  return {
    async create(snapshot) {
      return assertRecord(await call(basePath, {
        method: "POST",
    ...
    async list() {
      const result = await call(basePath, { method: "GET" });
    ...
    async get(id) {
      return assertRecord(await call(`${basePath}/${encodeURIComponent(id)}`, { method: "GET" }));
    },

    async update(id, snapshot, expectedRevision) {
      return assertRecord(await call(`${basePath}/${encodeURIComponent(id)}`, {
        method: "PATCH",
    ...
    async remove(id) {
      const result = await call(`${basePath}/${encodeURIComponent(id)}`, { method: "DELETE" });
